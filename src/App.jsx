...
// --- CONFIGURATION & HARDWARE ADDRESSES ---
const SERVICE_UUID = "4fafc201-1fb5-459e-8fcc-c5c9c331914b";
const CHARACTERISTIC_UUID = "beb5483e-36e1-4688-b7f5-ea07361b26a8";

// UWB Anchor Hardware Addresses
const A1 = "1786"; 
const A2 = "1685";
const A3 = "1584";

// --- SIGNAL PROCESSING (KALMAN FILTER) ---
const KALMAN_R = 6;     // Measurement Noise: Higher = smoother but more lag
const KALMAN_Q = 0.001; // Process Noise: Lower = more trust in the model vs sensors

// --- PHYSICAL CHARACTERISTICS ---
const PEN_LENGTH_MM = 140;   // Distance from UWB tag to the pen tip
const IMU_WRITING_GAIN = 10.0; // Multiplier to translate tilt to tip offset

// --- SENSOR THRESHOLDS ---
const PRESSURE_MAX = 4095;
const PRESSURE_THRESHOLD = 200; // Minimum pressure to trigger "Pen Down"

// --- STABILIZATION & LERP TUNING ---
const DEADZONE_PX = 0.8;   // Ignore movements smaller than this to prevent jitter
const SMOOTHING_SLOW = 0.15; // Smoothing factor for slow movements
const SMOOTHING_FAST = 0.8;  // Smoothing factor for quick movements

/**
 * Linear Interpolation: Transitions between two values based on alpha (amt)
 */
const lerp = (start, end, amt) => (1 - amt) * start + amt * end;

/**
 * 1D Kalman Filter for smoothing noisy sensor streams (X and Y coordinates)
 */
class KalmanFilter {
...
export default function SmartStrokeDashboard() {
  // --- DOM REFS ---
  const canvasRef = useRef(null); // Main drawing layer
  const cursorRef = useRef(null); // Ghost layer for real-time cursor/line previews

  // --- UI & CONNECTION STATE ---
  const [isConnected, setIsConnected] = useState(false);
  const [isCalibrated, setIsCalibrated] = useState(false);
  const [calibStep, setCalibStep] = useState(0); // 0: Origin, 1: Width
  const [anchorMap, setAnchorMap] = useState([]); // Stores relative (x,y) of UWB anchors

  // --- CANVAS COORDINATES & SCALING ---
  const [canvasDim, setCanvasDim] = useState({ w: 800, h: 600 });
  const [pixelsPerMm, setPixelsPerMm] = useState(1); // Maps physical MM to screen Pixels

  // --- DRAWING DATA ---
  const [pages, setPages] = useState([[]]); // History of completed strokes
  const [currentPageIndex, setCurrentPageIndex] = useState(0);
  const [selectedColor, setSelectedColor] = useState('#1e3a8a');
  const [toast, setToast] = useState({ show: false, message: '' });
  
  // High-frequency UI update state for telemetry (rotations, pressure)
  const [uiPenData, setUiPenData] = useState({ r: 1, i: 0, j: 0, k: 0, p: 0, down: false });

  // --- PERSISTENT SENSOR DATA (Ref avoids re-renders on every packet) ---
  const sensorDataRef = useRef({
    links: [], // UWB ranging distances
    pen: { r: 1, i: 0, j: 0, k: 0, down: false, p: 0, np: 0 } // IMU + Pressure
  });

  const currentStrokePoints = useRef([]); // Points in the active stroke
  const stabilizedPos = useRef({ x: 0, y: 0 }); // Current smoothed cursor position
  ...
  const centerRef = useRef(null); // Reference orientation for IMU (zero-point)
  const lastNpSignal = useRef(0); // Tracks "New Page" button state change
  const requestRef = useRef();
  const prevDownRef = useRef(false); // Detects "Pen Down" edge for stabilization reset

  // --- GEOMETRY & MATH HELPERS ---

  /**
   * Converts Quaternion (r, i, j, k) to Euler Angles (Yaw/Pitch)
   */
  ...
  /**
   * Calculates the shortest angular distance between two radians
   */
  const angleDiff = (a, b) => {
    let d = a - b;
  ...
  /**
   * Finds (x, y) coordinates of the pen based on 3 distances from known anchor points
   */
    ...
    try {
      ...
      char.addEventListener('characteristicvaluechanged', (e) => {
        ...
        try {
          ...
          // Logic for pen pressure and contact detection
          ...
          // Handle hardware "New Page" button click
  ...
  const handleCalibration = () => {
    const getD = (addr) => {
      ...
      return parseFloat(link?.R || 0) * 1000; // Convert to mm
    };
    const d1 = getD(A1), d2 = getD(A2), d3 = getD(A3);

    if (calibStep === 0) {
      // Step 1: Fix origin and measure distances to other anchors
      setAnchorMap([{ x: 0, y: 0 }, { d1_a2: d2, d1_a3: d3 }]);
      setCalibStep(1);
      triggerToast("Origin Fixed. Tap Anchor 2.");
    } else {
      // Step 2: Solve the triangle to find relative X/Y positions of all anchors
      ...
      // Map physical dimensions to virtual canvas size
  ...
  const animate = () => {
    if (isCalibrated && canvasRef.current && cursorRef.current && anchorMap.length === 3) {
      ...
      if (worldPos) {
        ...
        // Calculate how much the tip is displaced based on pen tilt
        ...
        // 5. Combine UWB position + IMU tip offset (Sensor Fusion)
        const targetX = stableX + pixelOffsetX;
        const targetY = stableY + pixelOffsetY;

        // Reset stabilizer immediately when pen touches surface to avoid "dragging" in
        ...
        // 6. Adaptive Stabilization (Eases towards target based on speed)
        ...
        if (pen.down) {
          ...
          // Pen lifted: Save current stroke to the page history
        ...
        // 8. Visual Feedback (Cursor & Linkage Line)
        cCtx.clearRect(0, 0, canvasDim.w, canvasDim.h);

        // Draw stable UWB center
        cCtx.beginPath();
        cCtx.arc(stableX, stableY, 3, 0, Math.PI * 2);
        cCtx.fillStyle = '#94a3b8';
        cCtx.fill();

        // Draw line connecting UWB center to predicted tip
        cCtx.beginPath();
  ...
  /**
   * Redraws the entire canvas when switching pages or resizing
   */
  useEffect(() => {
  ...
  return (
    /* UI Render Logic remains unchanged */
